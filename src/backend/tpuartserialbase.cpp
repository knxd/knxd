...
TPUARTBASESerialLayer2Driver::TPUARTBASESerialLayer2Driver (const char *dev,
...
TPUARTBASESerialLayer2Driver::~TPUARTBASESerialLayer2Driver ()
{
...
bool TPUARTBASESerialLayer2Driver::init ()
{
    struct termios t1;
    ...
    fd = open (m_dev.c_str(), O_RDWR | O_NOCTTY | O_NDELAY | O_SYNC);
    if (fd == -1)
        return false;
    ...
    fd = open (m_dev.c_str(), O_RDWR | O_NOCTTY | O_SYNC);
    if (fd == -1)
        return false;
    ...
    t1 = getTermiosSettings();
    cfsetospeed (&t1, getBaudRate());
    cfsetispeed (&t1, 0);
...
bool
TPUARTBASESerialLayer2Driver::addAddress (eibaddr_t addr)
{
    unsigned i;
    for (i = 0; i < indaddr (); i++)
        if (indaddr[i] == addr)
...
bool
TPUARTBASESerialLayer2Driver::addGroupAddress (eibaddr_t addr)
{
    unsigned i;
...
bool
TPUARTBASESerialLayer2Driver::removeAddress (eibaddr_t addr)
{
    unsigned i;
    for (i = 0; i < indaddr (); i++)
        if (indaddr[i] == addr)
...
bool
TPUARTBASESerialLayer2Driver::removeGroupAddress (eibaddr_t addr)
{
    unsigned i;
    for (i = 0; i < groupaddr (); i++)
        if (groupaddr[i] == addr)
...
bool TPUARTBASESerialLayer2Driver::openVBusmonitor ()
{
    vmode = 1;
    return 1;
}

bool TPUARTBASESerialLayer2Driver::closeVBusmonitor ()
{
    vmode = 0;
    return 1;
}
...
eibaddr_t
TPUARTBASESerialLayer2Driver::getDefaultAddr ()
{
    return addr;
}

bool
TPUARTBASESerialLayer2Driver::Connection_Lost ()
{
    return 0;
}

bool
TPUARTBASESerialLayer2Driver::Send_Queue_Empty ()
{
    return inqueue.isempty ();
}

void
TPUARTBASESerialLayer2Driver::Send_L_Data (LPDU * l)
{
...
LPDU *
TPUARTBASESerialLayer2Driver::Get_L_Data (pth_event_t stop)
{
    if (stop != NULL)
...
bool
TPUARTBASESerialLayer2Driver::enterBusmonitor ()
{
    uchar c = 0x05;
...
bool
TPUARTBASESerialLayer2Driver::leaveBusmonitor ()
{
    uchar c = 0x01;
...
bool
TPUARTBASESerialLayer2Driver::Open ()
{
    uchar c = 0x01;
...
bool
TPUARTBASESerialLayer2Driver::Close ()
{
    return 1;
}

void
TPUARTBASESerialLayer2Driver::RecvLPDU (const uchar * data, int len)
{
...
void
TPUARTBASESerialLayer2Driver::Run (pth_sem_t * stop1)
{
    uchar buf[255];
    int i;
    CArray in;
