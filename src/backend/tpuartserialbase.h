...
#ifndef TPUART_SERIAL_BASE_H
#define TPUART_SERIAL_BASE_H
#include <termios.h>
...
class TPUARTBASESerialLayer2Driver:public Layer2Interface, private Thread
{
protected:
  /** old serial config */
...
public:
    TPUARTBASESerialLayer2Driver (const char *dev, eibaddr_t addr, int flags,
                  Trace * tr);
  virtual ~TPUARTBASESerialLayer2Driver ();
  bool init ();
...
protected:
  virtual struct termios getTermiosSettings() = 0;
  virtual int getBaudRate() = 0;

  virtual void afterLPDUReceived() {}
  ///
  /// \brief gets called before the recv loop does anything,
  /// return value true means continue
  ///
  virtual bool beforeLoop(CArray &in) {return false;}

  virtual bool handlePackage(CArray &in) { return true; }
