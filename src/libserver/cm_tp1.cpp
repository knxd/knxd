      ...
      if (len + 8 != c.size())
        {
          if (c.size() == 23)
            {
              l->valid_length = 0;
              l->data.set (c.data() + 7, 8);
            }
          else
            return nullptr;
        }
      else
        l->data.set (c.data() + 7, len);
    }
...
CArray L_Data_to_CM_TP1 (const LDataPtr & p)
{
  ...
  assert (p->data.size() <= 0xff);
  assert ((p->hop_count & 0xf8) == 0);

  CArray pdu;
  uint8_t len = p->data.size() - 1;
  if (len <= 0x0f)
    {
      /* L_Data_Standard Frame */
      pdu.resize (7 + p->data.size());
      ...
      pdu[5] =
        ...
        ((p->hop_count & 0x07) << 4) |
        (len & 0x0f);
      pdu.setpart (p->data.data(), 6, 1 + (len & 0x0f));
    }
  else
    {
      /* L_Data_Extended Frame */
      pdu.resize (8 + p->data.size());
      ...
      pdu[6] = (p->data.size() - 1) & 0xff;
      pdu.setpart (p->data.data(), 7, 1 + (len & 0xff));
    }
