...
A_Broadcast::A_Broadcast (ClientConnection * cc)
{
  ...
  c = T_BroadcastPtr(new T_Broadcast (con->t, con->buf[4] != 0));
  if (!c->init (con->l3))
    {
...
A_Group::A_Group (ClientConnection * cc)
{
  ...
  c = T_GroupPtr(new T_Group (cc->t, (con->buf[2] << 8) | (con->buf[3]),
		 con->buf[4] != 0));
  if (!c->init (con->l3))
    {
...
A_TPDU::A_TPDU (ClientConnection * cc)
{
  ...
  c = T_TPDUPtr(new T_TPDU (con->t, (con->buf[2] << 8) | (con->buf[3])));
  if (!c->init (con->l3))
    {
...
A_Individual::A_Individual (ClientConnection * cc)
{
  ...
  c = T_IndividualPtr(
    new T_Individual (con->t, (con->buf[2] << 8) | (con->buf[3]),
		      con->buf[4] != 0));
  if (!c->init (con->l3))
    {
...
A_Connection::A_Connection (ClientConnection * cc)
{
  ...
  c = T_ConnectionPtr(new T_Connection (con->t, (con->buf[2] << 8) | (con->buf[3])));
  if (!c->init (con->l3))
    {
...
A_GroupSocket::A_GroupSocket (ClientConnection * cc)
{
  ...
  c = GroupSocketPtr(new GroupSocket (con->t, con->buf[4] != 0));
  if (!c->init (con->l3))
    {
