...
parse_opt (int key, char *arg, struct argp_state *state)
{
  ...
  switch (key)
    ...
    case OPT_BACK_TUNNEL_NOQUEUE: // obsolete
      ERRORPRINTF (arguments->tracer(), E_WARNING | 41, 0, "The option '--no-tunnel-client-queuing' is obsolete.");
      ERRORPRINTF (arguments->tracer(), E_WARNING | 42, 0, "Please use '--send-delay=30'.");
      arguments->l2opts.send_delay = 30; // msec
      break;
    case OPT_BACK_EMI_NOQUEUE: // obsolete
      ERRORPRINTF (arguments->tracer(), E_WARNING | 43, 0, "The option '--no-emi-send-queuing' is obsolete.");
      ERRORPRINTF (arguments->tracer(), E_WARNING | 44, 0, "Please use '--send-delay=500'.");
